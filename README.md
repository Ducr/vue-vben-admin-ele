...
基于 **Vue Vben Admin v5.5.9** 独立拆分而来的 Element Plus 版本单页应用（SPA）。

## 📖 项目简介

`vue-vben-admin-ele` 是从原始 [Vue Vben Admin](https://github.com/vbenjs/vue-vben-admin) monorepo 架构中独立拆分出来的 **Element Plus** 版本。本项目保留了 Vue Vben Admin 的所有核心功能特性，但完全脱离 pnpm workspace 和 monorepo 依赖，可作为独立项目单独部署、测试和发布。
...
- 🎨 **基于 Element Plus 2.x** - 使用 Element Plus 作为 UI 组件库
...
- 🎭 **按需导入** - 使用 `unplugin-element-plus` 实现 Element Plus 样式的按需导入
...
├── docs/              # 文档站点
├── public/            # 静态资源
├── vite.config.ts     # Vite 配置（包含 Element Plus 按需导入）
...
- **Element Plus**: [Element Plus 文档](https://element-plus.org)
